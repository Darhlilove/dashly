      ...
      <div className="text-center py-8 sm:py-12 px-4">
        ...
        <div className="flex flex-col sm:flex-row gap-3 justify-center items-center">
          <div className="flex items-center text-xs sm:text-sm text-gray-400">
            ...
            <span>Upload CSV → Ask question → Save dashboard</span>
          </div>
        </div>
      </div>
    </div>
...
const DashboardGridSkeleton: React.FC<{ className?: string }> = ({
  ...
  return (
    <div className={`w-full ${className}`} role="status" aria-label="Loading dashboards">
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4 sm:gap-6">
        {Array.from({ length: 6 }).map((_, index) => (
          ...
          >
            ...
            {/* Chart skeleton */}
            <div className="h-24 sm:h-32 mb-3 bg-gray-100 rounded"></div>
      ...
      <span className="sr-only">Loading saved dashboards...</span>
    </div>
  );
};
